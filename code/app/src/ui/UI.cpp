...
void UI::init() {
  if (glfwInit() == GLFW_FALSE) throw "GLFW fail to initialize.";
...
#pragma region _INIT_
  ...
  views.push_back(
    new ComponentProgressView({}, {240, 100}, uiContext, manager));
...
void UI::run() {
  ...
  io.Fonts->AddFontFromFileTTF("c:/windows/fonts/msyh.ttc", 16.0f, NULL,
  ...
  getServer().logger.success(
    "欢迎使用(*^_^*), 程序还有很多不完善的地方, 遇到问题希望大家积极反映.");
