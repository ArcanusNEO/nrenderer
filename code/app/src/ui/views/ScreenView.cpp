...
ScreenView::ScreenView(const Vec2& position, const Vec2& size,
...
bool ScreenView::isPreviewSizeChange() {
  bool f =
    (previewWidth != prePreviewWidth || previewHeight != prePreviewHeight);
  return f;
}
...
void ScreenView::draw() {
  ...
  ImGui::BeginChild("Screen Content", size, false,
  ...
  if (ImGui::BeginCombo(
        "##ShrinkLevel", shrinkLevelsLabel[shrinkLevel].c_str())) {
    for (int i = 0; i < 8; i++) {
  ...
  if (viewType == ViewType::PREVIEW) {
    ...
    if (previewCoordinateType == CoordinateType::LEFT_HANDED) {
      if (ImGui::Button("Left-handed", {100, 22}))
        previewCoordinateType = CoordinateType::RIGHT_HANDED;
    } else {
      if (ImGui::Button("Right-handed", {100, 22}))
        previewCoordinateType = CoordinateType::LEFT_HANDED;
    }
  }
}
...
void ScreenView::genFB() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, previewWidth, previewHeight, 0,
  ...
  glRenderbufferStorage(
    GL_RENDERBUFFER, GL_DEPTH_COMPONENT, previewWidth, previewHeight);
  glBindRenderbuffer(GL_RENDERBUFFER, 0);

  glGenFramebuffers(1, &glFBO);
  glBindFramebuffer(GL_FRAMEBUFFER, glFBO);
  glFramebufferTexture2D(
    GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, previewResult, 0);
  glFramebufferRenderbuffer(
    GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, glRBO);
...
void ScreenView::drawGlPreview() {
  ...
  Mat4x4 projection = glm::perspective(
    glm::radians(camera.fov), camera.aspect, 0.1f, 10000000.0f);
  ...
  if (uiContext.previewMode == UIContext::PreviewMode::PREVIEW_MODEL) {
    if (uiContext.previewModel != -1 &&
      uiContext.previewModel < asset.modelItems.size()) {
    ...
    if (uiContext.previewNode != -1 &&
      uiContext.previewNode < asset.nodeItems.size()) {
      nodeShader.setVec4("drawColor", selectedColor);
  ...
  if (uiContext.previewMode == UIContext::PreviewMode::PREVIEW_LIGHT) {
    if (uiContext.previewLight != -1 &&
      uiContext.previewLight < asset.lightItems.size()) {
...
void ScreenView::previewLight(const LightItem& l) {
  ...
  if (l.light->type == Light::Type::AREA) {
    ...
    float dis = glm::length(
      camera.position - asset.pointLights[l.light->entity]->position);
