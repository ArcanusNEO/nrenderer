...
namespace NRenderer {
SceneView::SceneView(const Vec2& position, const Vec2& size,
...
void SceneView::cameraSetting() {
  ...
  ImGui::InputScalar(
    "Aspect", ImGuiDataType_Float, &camera.aspect, &floatStep, NULL);
  ImGui::InputScalar(
    "Aperture", ImGuiDataType_Float, &camera.aperture, &floatStep, NULL);
  ImGui::InputScalar("FocusDistance", ImGuiDataType_Float,
    &camera.focusDistance, &floatStep, NULL);
}
...
void SceneView::renderSetting() {
  ...
  ImGui::InputScalar(
    "Width", ImGuiDataType_U32, &rs.width, &intStep, NULL, "%u");
  ImGui::InputScalar(
    "Height", ImGuiDataType_U32, &rs.height, &intStep, NULL, "%u");
  ImGui::InputScalar(
    "Depth", ImGuiDataType_U32, &rs.depth, &intStep, NULL, "%u");
...
void SceneView::ambientSetting() {
  ...
  if (ImGui::BeginCombo("Type##AmbientSettings", typeStr[curr].c_str())) {
    for (int i = 0; i < 2; i++) {
      ...
      if (ImGui::Selectable(
            (typeStr[i] + "##AmbientTypeItem").c_str(), &selected)) {
  ...
  if (curr == 0) {
    ...
    if (ImGui::BeginCombo("Map Texture##AmbientEMAPTex",
          v.valid() ?
...
void SceneView::componentSetting() {
  ...
  string comboPre =
    currComponentSelected != -1 && currComponentSelected < components.size() ?
  ...
  if (ImGui::BeginCombo("Render Component##SceneView", comboPre.c_str())) {
    for (int i = 0; i < components.size(); i++) {
      ...
      if (ImGui::Selectable(
            (to_string(i + 1) + ". " + components[i].name + "##SceneView")
              .c_str(),
            &selected)) {
  ...
  if (currComponentSelected != -1 &&
    currComponentSelected < components.size()) {
  ...
  if (ImGui::Button("Render")) {
    if (currComponentSelected != -1 &&
      ...
      SceneBuilder sceneBuilder {manager.assetManager.asset, rs.renderSettings,
        rs.ambientSettings, rs.camera};
      componentManager.exec<RenderComponent>(
        components[currComponentSelected], sceneBuilder.build());
    } else {
