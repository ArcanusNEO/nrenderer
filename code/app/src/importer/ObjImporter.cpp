...
namespace NRenderer {
inline TextureItem loadTexture(const string& filePath, const string& fileName) {
...
bool ObjImporter::parseMtl(Asset& asset, const string& path, ifstream& file,
  ...
  while (getline(file, currLine)) {
    ...
    if (token == "newmtl") {
      ...
      currMaterialItem->material->registerProperty(
        "diffuseColor", PW::RGBType {diffuseColor});
    } else if (token == "ks") {
      float f1, f2, f3;
      ss >> f1 >> f2 >> f3;
      auto specularColor = Vec3 {f1, f2, f3};
      currMaterialItem->material->registerProperty(
        "specularColor", PW::RGBType {specularColor});
    } else if (token == "ns") {
      float f;
      ss >> f;
      auto specularEx = f;
      currMaterialItem->material->registerProperty(
        "specularEx", PW::FloatType {f});
    } else if (token == "d") {
      float f;
      ss >> f;
    ...
    } else if (token == "map_ka" || token == "map_d" || token == "disp" ||
      token == "decal") {
      // 不支持
...
bool ObjImporter::import(Asset& asset, const string& path) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (v != -1) {
          ...
          if (finded != pMap->end()) {
            ...
            auto idx =
              asset.meshes[currNodePtr->node->entity]->positions.size();
            vpi[i] = idx;
            pMap->insert({v, idx});
            asset.meshes[currNodePtr->node->entity]->positions.push_back(
              positions[v - 1]);
          }
          asset.meshes[currNodePtr->node->entity]->positionIndices.push_back(
            vpi[i]);
        }
        if (t != -1) {
        ...
        if (n != -1) {
          ...
          if (finded != nMap->end()) {
            ...
            vni[i] = idx;
            nMap->insert({n, idx});
            asset.meshes[currNodePtr->node->entity]->normals.push_back(
              normals[n - 1]);
          }
          asset.meshes[currNodePtr->node->entity]->normalIndices.push_back(
            vni[i]);
        }
      }
      if (!ss.eof()) {
  ...
  if (!successFlag) {
    asset.modelItems.erase(
      asset.modelItems.begin() + beginModel, asset.modelItems.end());
    asset.nodeItems.erase(
      asset.nodeItems.begin() + beginNode, asset.nodeItems.end());
    asset.materialItems.erase(
      asset.materialItems.begin() + beginMaterial, asset.materialItems.end());
    asset.textureItems.erase(
      asset.textureItems.begin() + beginTexture, asset.textureItems.end());

    asset.spheres.erase(asset.spheres.begin() + beginSph, asset.spheres.end());
    asset.triangles.erase(
      asset.triangles.begin() + beginTri, asset.triangles.end());
    ...
    asset.lightItems.erase(
      asset.lightItems.begin() + beginLight, asset.lightItems.end());
    asset.pointLights.erase(
      asset.pointLights.begin() + beginPnt, asset.pointLights.end());
    asset.areaLights.erase(
      asset.areaLights.begin() + beginArea, asset.areaLights.end());
    asset.directionalLights.erase(asset.directionalLights.begin() + beginDir,
      asset.directionalLights.end());
    asset.spotLights.erase(
      asset.spotLights.begin() + beginSpt, asset.spotLights.end());
  }
