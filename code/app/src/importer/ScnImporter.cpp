...
namespace NRenderer {
bool ScnImporter::parseMtl(
  Asset& asset, ifstream& file, map<string, size_t>& mtlMap) {
  string currline;
  ...
  while (getline(file, currline)) {
    ...
    if (token == "") continue;
      ...
      if (type == "Int") {
        ...
        float v1, v2, v3;
        ss >> v1 >> v2 >> v3;
        spMaterial->registerProperty(key,
          PW::Vec3Type {
            Vec3 {v1, v2, v3}
        });
      } else if (type == "Vec4") {
...
bool ScnImporter::parseMdl(
  ...
  while (getline(file, currline)) {
    ...
    if (token == "") continue;
      ...
      asset.modelItems[asset.modelItems.size() - 1].model->nodes.push_back(
        asset.nodeItems.size());
