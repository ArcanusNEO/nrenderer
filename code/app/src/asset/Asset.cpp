...
void Asset::genPreviewGlBuffersPerNode(NodeItem& node) {
  ...
  if (np.type == T::TRIANGLE) {
    ...
    for (int i = 0; i <= count; i++) {
      Vec3 p = (s.position +
        Vec3 {s.radius * cos(float(i) / float(count) * 6.28),
          s.radius * sin(float(i) / float(count) * 6.28), 0});
      bf.push_back(p);
    }
    for (int i = 0; i <= count; i++) {
      Vec3 p = (s.position +
        Vec3 {s.radius * cos(float(i) / float(count) * 6.28), 0,
          s.radius * sin(float(i) / float(count) * 6.28)});
      bf.push_back(p);
    }
    ...
    for (int i = 0; i <= count; i++) {
      Vec3 p = (s.position +
        Vec3 {0, s.radius * sin(float(i) / float(count) * 6.28),
          s.radius * cos(float(i) / float(count) * 6.28)});
      bf.push_back(p);
    }
    glGenBuffers(1, &node.glVBO);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, node.glVBO);
    glBufferData(
      GL_ARRAY_BUFFER, sizeof(Vec3) * bf.size(), &bf[0], GL_DYNAMIC_DRAW);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, node.glVBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3) * m.positions.size(),
      &m.positions[0], GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, node.glEBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
      sizeof(unsigned int) * m.positionIndices.size(), &m.positionIndices[0],
      GL_DYNAMIC_DRAW);
...
void Asset::genPreviewGlBuffersPerLight(LightItem& light) {
  ...
  if (lp.type == T::POINT) {
    ...
    const int count = 8;
    for (int i = 0; i < count; i++) {
      Vec3 p {l * cos(float(i) / float(count) * 2 * 3.1415926f),
        l * sin(float(i) / float(count) * 2 * 3.1415926f), 0};
      ps.push_back({0, 0, 0});
      ps.push_back(p);
    }
    ...
    glBufferData(
      GL_ARRAY_BUFFER, sizeof(Vec3) * ps.size(), &ps[0], GL_STATIC_DRAW);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, light.glVBO);
    glBufferData(
      GL_ARRAY_BUFFER, sizeof(Vec3) * ps.size(), &ps[0], GL_DYNAMIC_DRAW);
