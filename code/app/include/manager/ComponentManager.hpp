...
public:
  enum class State { IDLING, READY, RUNNING, FINISH };
...
public:
  ...
  void exec(const ComponentInfo& componentInfo, Args... args) {
    auto component = getServer().componentFactory.createComponent<Interface>(
      componentInfo.type, componentInfo.name);
    activeComponent = componentInfo;
    this->state = State::READY;
    try {
      t = thread(
        &Interface::exec, component,
        [this]() -> void {
