...
namespace NRenderer {
bool ComponentFactory::registerComponent(const string& type, const string& name,
  const string& desc, function<SharedInstance()> constructor) {
  ...
  if (ncMap.find(name) == ncMap.end()) {
    ...
    throw "Target Component: " + getId(type, name) +
      " has already been registered. Check your registration info.";
    return false;
  }
}

void ComponentFactory::unregisterComponent(
  const string& type, const string& name) {
  auto& ncMap = constructors[type];
  ncMap.erase(name);
}

SharedInstance ComponentFactory::createInstance(
  const string& type, const string& name) {
  auto& ncMap = constructors[type];
  if (ncMap.find(name) != ncMap.end()) {
