...
struct Property {
  ...
  enum class Type { INT = 0x0, FLOAT, RGB, RGBA, VEC3, VEC4, TEXTURE_ID };
  #define Types                                                                \
    Wrapper::IntType, Wrapper::FloatType, Wrapper::RGBType, Wrapper::RGBAType, \
      Wrapper::Vec3Type, Wrapper::Vec4Type, Wrapper::TextureIdType
...
public:
  ...
  inline bool hasProperty(const string& key) {
    auto it = find_if(properties.begin(), properties.end(),
      [&key](const Property& p1) -> bool { return p1.key == key; });
