...
void ImGui::TextEx(
  const char* text, const char* text_end, ImGuiTextFlags flags) {
...
void ImGui::BulletTextV(const char* fmt, va_list args) {
  ...
  const ImVec2 total_size = ImVec2(g.FontSize +
    ...
    if (!(flags & ImGuiButtonFlags_NoKeyModifiers) ||
      ...
      if (mouse_button_clicked != -1 && g.ActiveId != id) {
        ...
        if ((flags & ImGuiButtonFlags_PressedOnClick) ||
          ...
          pressed = true;
          if (flags & ImGuiButtonFlags_NoHoldingActiveId) ClearActiveID();
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
  ...
  if (held && allow_interaction && grab_h_norm < 1.0f) {
    ...
    if (g.ActiveIdIsJustActivated) {
      ...
      if (seek_absolute) g.ScrollbarClickDeltaToGrabCenter = 0.0f;
      else
...
void ImGui::ProgressBar(
  ...
  RenderRectFilledRangeH(window->DrawList, bb,
    GetColorU32(ImGuiCol_PlotHistogram), 0.0f, fraction, style.FrameRounding);
...
void ImGui::NewLine() {
  ...
  if (window->DC.CurrLineSize.y >
    ...
    ItemSize(ImVec2(0, 0));
  else ItemSize(ImVec2(0.0f, g.FontSize));
...
static int IMGUI_CDECL ShrinkWidthItemComparer(
...
bool ImGui::BeginCombo(
  ...
  if (has_window_size_constraint) {
    ...
    SetNextWindowSizeConstraints(ImVec2(w, 0.0f),
      ImVec2(
        FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
  }
  ...
  ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize |
    ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar |
...
static bool Items_SingleStringGetter(
  ...
  switch (data_type) {
    ...
    case ImGuiDataType_S16 :
      return DataTypeCompareT<ImS16>(
        (const ImS16*) arg_1, (const ImS16*) arg_2);
    case ImGuiDataType_U16 :
      return DataTypeCompareT<ImU16>(
        (const ImU16*) arg_1, (const ImU16*) arg_2);
    case ImGuiDataType_S32 :
      return DataTypeCompareT<ImS32>(
        (const ImS32*) arg_1, (const ImS32*) arg_2);
    case ImGuiDataType_U32 :
      return DataTypeCompareT<ImU32>(
        (const ImU32*) arg_1, (const ImU32*) arg_2);
    case ImGuiDataType_S64 :
      return DataTypeCompareT<ImS64>(
        (const ImS64*) arg_1, (const ImS64*) arg_2);
    case ImGuiDataType_U64 :
      return DataTypeCompareT<ImU64>(
        (const ImU64*) arg_1, (const ImU64*) arg_2);
    case ImGuiDataType_Float :
      return DataTypeCompareT<float>(
        (const float*) arg_1, (const float*) arg_2);
    case ImGuiDataType_Double :
      return DataTypeCompareT<double>(
        (const double*) arg_1, (const double*) arg_2);
    case ImGuiDataType_COUNT : break;
  }
  IM_ASSERT(0);
  return 0;
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed,
  ...
  if (is_logarithmic) {
    ...
    v_cur = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type,
      v_new_parametric, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon,
      zero_deadzone_halfsize);
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v,
  ...
  switch (data_type) {
    ...
    case ImGuiDataType_Double :
      return DragBehaviorT<double, double, double>(data_type, (double*) p_v,
        v_speed, p_min ? *(const double*) p_min : -DBL_MAX,
  ...
  if (!temp_input_is_active) {
    ...
    if (focus_requested || clicked || double_clicked || g.NavActivateId == id ||
      ...
      if (temp_input_allowed &&
        (focus_requested || (clicked && g.IO.KeyCtrl) || double_clicked ||
          g.NavInputId == id)) {
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min,
  ...
  value_changed |= DragScalar("##max", ImGuiDataType_Float, v_current_max,
    v_speed, &max_min, &max_max, format_max ? format_max : format, max_flags);
  PopItemWidth();
  SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data,
  ...
  return DragScalar(
    label, data_type, p_data, v_speed, p_min, p_max, format, drag_flags);
}
...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min,
  TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon,
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min,
  TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon,
  float zero_deadzone_halfsize) {
  if (v_min == v_max) return (TYPE) 0.0f;
  const bool is_decimal =
    (data_type == ImGuiDataType_Float) || (data_type == ImGuiDataType_Double);
  ...
  if (is_logarithmic) {
    ...
    if (t <= 0.0f) result = v_min;
    else if (t >= 1.0f) result = v_max;
    else {
        ...
        0.0f)  // Range crosses zero, so we have to do this in two parts
      {
        ...
        result = (TYPE) -
          (-v_max_fudged *
            ImPow(
              -v_min_fudged / -v_max_fudged, (FLOATTYPE) (1.0f - t_with_flip)));
      else
  ...
  if (g.ActiveId == id) {
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
        ...
        clicked_t = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type,
          *v, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon,
          zero_deadzone_halfsize);
              ...
              0.0f))  // This is to avoid applying the saturation when already past the limits
        {
          ...
          float new_clicked_t =
            ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, v_new,
              v_min, v_max, is_logarithmic, logarithmic_zero_epsilon,
              zero_deadzone_halfsize);
    ...
    if (set_new_value) {
      TYPE v_new = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type,
        clicked_t, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon,
        zero_deadzone_halfsize);
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id,
  ...
  switch (data_type) {
    case ImGuiDataType_S8 : {
      ImS32 v32 = (ImS32) * (ImS8*) p_v;
      bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32,
        &v32, *(const ImS8*) p_min, *(const ImS8*) p_max, format, flags,
        out_grab_bb);
      if (r) *(ImS8*) p_v = (ImS8) v32;
      return r;
    }
    case ImGuiDataType_U8 : {
      ImU32 v32 = (ImU32) * (ImU8*) p_v;
      bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32,
        &v32, *(const ImU8*) p_min, *(const ImU8*) p_max, format, flags,
        out_grab_bb);
      if (r) *(ImU8*) p_v = (ImU8) v32;
      return r;
    }
    case ImGuiDataType_S16 : {
      ImS32 v32 = (ImS32) * (ImS16*) p_v;
      bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32,
        &v32, *(const ImS16*) p_min, *(const ImS16*) p_max, format, flags,
        out_grab_bb);
      if (r) *(ImS16*) p_v = (ImS16) v32;
      return r;
    }
    case ImGuiDataType_U16 : {
      ImU32 v32 = (ImU32) * (ImU16*) p_v;
      bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32,
        &v32, *(const ImU16*) p_min, *(const ImU16*) p_max, format, flags,
        out_grab_bb);
      if (r) *(ImU16*) p_v = (ImU16) v32;
      return r;
    }
    ...
    case ImGuiDataType_Double :
      ...
      return SliderBehaviorT<double, double, double>(bb, id, data_type,
        (double*) p_v, *(const double*) p_min, *(const double*) p_max, format,
        flags, out_grab_bb);
    case ImGuiDataType_COUNT : break;
  }
  IM_ASSERT(0);
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size,
  ...
  const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data,
    p_min, p_max, format, flags | ImGuiSliderFlags_Vertical, &grab_bb);
  if (value_changed) MarkItemEdited(id);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type,
  ...
  return SliderScalar(
    label, data_type, p_data, p_min, p_max, format, slider_flags);
}
...
bool ImGui::SliderScalarN(const char* label, ImGuiDataType data_type, void* v,
  ...
  return SliderScalarN(
    label, data_type, v, components, v_min, v_max, format, slider_flags);
}
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type,
  void* p_data, const void* p_step, const void* p_step_fast, const char* format,
  ImGuiInputTextFlags flags) {
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems) return false;
  ...
  if (p_step != NULL) {
    ...
    if (
      InputText("", buf, IM_ARRAYSIZE(buf),
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf,
  ...
  if (
    is_multiline)  // Open group before calling GetID() because groups tracks id created within their scope,
    BeginGroup();
  const ImGuiID id = window->GetID(label);
  const ImVec2 label_size = CalcTextSize(label, NULL, true);
  ...
  const ImVec2 total_size = ImVec2(frame_size.x +
      (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f),
    frame_size.y);
  ...
  if (render_cursor || render_selection) {
    ...
    if (render_cursor) {
      ...
      ImRect cursor_screen_rect(cursor_screen_pos.x,
        cursor_screen_pos.y - g.FontSize + 0.5f, cursor_screen_pos.x + 1.0f,
  ...
  IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags);
  if ((flags & ImGuiInputTextFlags_EnterReturnsTrue) != 0) return enter_pressed;
  ...
  if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
    ...
    if (ColorButton("##ColorButton", col_v4, flags)) {
      if (!(flags & ImGuiColorEditFlags_NoPicker)) {
        ...
        OpenPopup("picker");
        SetNextWindowPos(
          window->DC.LastItemRect.GetBL() + ImVec2(-1, style.ItemSpacing.y));
      }
    }
    if (!(flags & ImGuiColorEditFlags_NoOptions))
      OpenPopupOnItemClick("context");
...
bool ImGui::ColorPicker4(const char* label, float col[4],
  ...
  if (!(flags & ImGuiColorEditFlags_NoSidePreview)) {
    ...
    ImGuiColorEditFlags sub_flags_to_forward = ImGuiColorEditFlags__InputMask |
  ...
  ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(ImVec4(R, G, B,
    style
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    for (int n = 0; n < 6; n++) {
      ...
      ShadeVertsLinearColorGradientKeepAlpha(draw_list, vert_start_idx,
        vert_end_idx, gradient_p0, gradient_p1, col_hues[n], col_hues[n + 1]);
    }
    ...
    draw_list->AddRectFilledMultiColor(picker_pos,
      picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0, 0, col_black,
      col_black);
    RenderFrameBorder(
  ...
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) {
    ...
    if (col_source.w < 1.0f)
      RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min,
        bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off),
        rounding);
    else
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags) {
  ...
  if (allow_opt_datatype) {
    ...
    if (RadioButton("0..255", (opts & ImGuiColorEditFlags_Uint8) != 0))
      opts =
        (opts & ~ImGuiColorEditFlags__DataTypeMask) | ImGuiColorEditFlags_Uint8;
    if (RadioButton("0.00..1.00", (opts & ImGuiColorEditFlags_Float) != 0))
      opts =
        (opts & ~ImGuiColorEditFlags__DataTypeMask) | ImGuiColorEditFlags_Float;
  }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  if (is_mouse_x_over_arrow) button_flags |= ImGuiButtonFlags_PressedOnClick;
  else if (flags & ImGuiTreeNodeFlags_OpenOnDoubleClick)
    button_flags |= ImGuiButtonFlags_PressedOnClickRelease |
      ImGuiButtonFlags_PressedOnDoubleClick;
  else button_flags |= ImGuiButtonFlags_PressedOnClickRelease;
  ...
  if (display_frame) {
    ...
    if (flags & ImGuiTreeNodeFlags_Bullet)
      RenderBullet(window->DrawList,
        ImVec2(
          text_pos.x - text_offset_x * 0.5f, text_pos.y + g.FontSize * 0.5f),
        text_col);
    else if (!is_leaf)
      RenderArrow(window->DrawList,
...
void ImGui::EndMenuBar() {
  ...
  if (NavMoveRequestButNoResultYet() &&
    ...
    if (nav_earliest_child->ParentWindow == window &&
      ...
      IM_ASSERT(
        window->DC.NavLayerActiveMaskNext & (1 << layer));  // Sanity check
      FocusWindow(window);
      SetNavIDWithRectRel(
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
  ...
  ImGuiWindowFlags flags = ImGuiWindowFlags_ChildMenu |
  ...
  if (menuset_is_open)
    g.NavWindow =
      window;  // Odd hack to allow hovering across menus of a same menu-set (otherwise we wouldn't be able to hover parent)
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    pressed = Selectable(label, menu_is_open,
      ...
      ImVec2(w, 0.0f));
    PopStyleVar();
    window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x *
      (-1.0f +
    ...
    RenderArrow(window->DrawList,
      pos +
        ImVec2(
          window->DC.MenuColumns.Pos[2] + extra_w + g.FontSize * 0.30f, 0.0f),
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    if (selected)
      RenderCheckMark(window->DrawList,
        pos +
          ImVec2(window->DC.MenuColumns.Pos[2] + extra_w + g.FontSize * 0.40f,
...
static void TabBarScrollToTab(
  ImGuiTabBar* tab_bar, ImGuiTabItem* tab, ImGuiTabBarSection* sections);
...
static int IMGUI_CDECL TabItemComparerBySection(
  const void* lhs, const void* rhs) {
...
static int IMGUI_CDECL TabItemComparerByBeginOrder(
  const void* lhs, const void* rhs) {
  const ImGuiTabItem* a = (const ImGuiTabItem*) lhs;
  const ImGuiTabItem* b = (const ImGuiTabItem*) rhs;
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar) {
  ...
  float x = ImMax(
    tab_bar->BarRect.Min.x, tab_bar->BarRect.Max.x - scrolling_buttons_width);
...
bool ImGui::BeginTabItem(
  ...
  if (ret && !(flags & ImGuiTabItemFlags_NoPushId)) {
    ...
    PushOverrideID(
      tab
        ->ID);  // We already hashed 'label' so push into the ID stack directly instead of doing another hash through PushID(label)
  }
  return ret;
}
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open,
  ...
  if (flags & ImGuiTabItemFlags_NoCloseButton) p_open = NULL;
  else if (p_open == NULL) flags |= ImGuiTabItemFlags_NoCloseButton;
  ...
  const bool is_central_section =
    (tab->Flags & (ImGuiTabItemFlags_Leading | ImGuiTabItemFlags_Trailing)) ==
    0;
  size.x = tab->Width;
  if (is_central_section)
    window->DC.CursorPos = tab_bar->BarRect.Min +
      ImVec2(IM_FLOOR(tab->Offset - tab_bar->ScrollingAnim), 0.0f);
  else window->DC.CursorPos = tab_bar->BarRect.Min + ImVec2(tab->Offset, 0.0f);
  ImVec2 pos = window->DC.CursorPos;
  ImRect bb(pos, pos + size);
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button) {
  ...
  else size.x += g.Style.FramePadding.x + 1.0f;
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
  ...
  if (close_button_visible) {
    ...
    if (CloseButton(close_button_id,
          ImVec2(
            bb.Max.x - frame_padding.x * 2.0f - close_button_sz, bb.Min.y)))
  ...
  RenderTextEllipsis(draw_list, text_ellipsis_clip_bb.Min,
    text_ellipsis_clip_bb.Max, text_pixel_clip_bb.Max.x, ellipsis_max_x, label,
    NULL, &label_size);
