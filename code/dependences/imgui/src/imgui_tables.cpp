...
inline ImGuiTableFlags TableFixFlags(
  ImGuiTableFlags flags, ImGuiWindow* outer_window) {
...
bool ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count,
  ...
  table->IsOuterRectMinFitX = (outer_size.x == 0.0f) &&
    (use_child_window ==
...
static void TableSetupColumnFlags(
  ...
  if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0) {
    ...
    else flags |= ImGuiTableColumnFlags_WidthStretch;
  } else {
...
void ImGui::TableUpdateLayout(ImGuiTable* table) {
  ...
  table->MinColumnWidth = ImMax(1.0f,
  ...
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
    ...
    if (column->Flags & ImGuiTableColumnFlags_WidthFixed) {
      ...
      if (column->AutoFitQueue != 0x00 || column->StretchWeight < 0.0f ||
        ...
        else column->StretchWeight = 1.0f;
      }
  ...
  const ImRect mouse_hit_rect(table->OuterRect.Min.x, table->OuterRect.Min.y,
    ...
    ImMax(
      table->OuterRect.Max.y, table->OuterRect.Min.y + table->LastOuterHeight));
  ...
  if (table->RightMostStretchedColumn != -1) table->IsOuterRectMinFitX = false;
...
void ImGui::TableUpdateBorders(ImGuiTable* table) {
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
    ...
    if (table->FreezeColumnsCount > 0)
      if (column->MaxX <
        table
          ->Columns[table->DisplayOrderToIndex[table->FreezeColumnsCount - 1]]
          .MaxX)
        continue;
...
void ImGui::EndTable() {
  ...
  IM_ASSERT(
    table != NULL && "Only call EndTable() if BeginTable() returns true!");
  ...
  IM_ASSERT(
    outer_window == inner_window || outer_window == inner_window->ParentWindow);
  ...
  if (table->ResizedColumn != -1 &&
    ...
    const float new_width = ImFloor(new_x2 - column->MinX -
      table->CellSpacingX1 - table->CellPaddingX * 2.0f);
  ...
  const float outer_width = table->IsOuterRectMinFitX ?
    table->WorkRect.GetWidth() :
    table->ColumnsAutoFitWidth;
  ...
  if (inner_window == outer_window && !(flags & ImGuiTableFlags_NoHostExtendY))
    outer_window->DC.CursorMaxPos.y =
      ImMax(outer_window->DC.CursorMaxPos.y, inner_content_max_y);
...
ImGuiID ImGui::TableGetColumnResizeID(
  const ImGuiTable* table, int column_n, int instance_no) {
  ...
  if (is_visible) {
    ...
    if (draw_cell_bg_color) {
      ...
      for (ImGuiTableCellData* cell_data = &table->RowCellData[0];
        ...
        cell_bg_rect.Min.x = ImMax(cell_bg_rect.Min.x,
          column->ClipRect.Min
            .x);  // So that first column after frozen one gets clipped
    ...
    if (draw_strong_bottom_border && bg_y2 >= table->BgClipRect.Min.y &&
      ...
      window->DrawList->AddLine(ImVec2(table->BorderX1, bg_y2),
        ImVec2(table->BorderX2, bg_y2), table->BorderColorStrong, border_size);
  }
...
float ImGui::TableGetMaxColumnWidth(const ImGuiTable* table, int column_n) {
  ...
  const float min_column_distance = table->MinColumnWidth +
    table->CellPaddingX * 2.0f + table->CellSpacingX1 + table->CellSpacingX2;
  if (table->Flags & ImGuiTableFlags_ScrollX) {
...
float ImGui::TableGetColumnWidthAuto(
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table) {
  ...
  const int channels_total = channels_for_bg +
    (channels_for_row * freeze_row_multiplier) + channels_for_dummy;
  ...
  if (table->Flags & ImGuiTableFlags_BordersOuter) {
    ...
    if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
      ...
      inner_drawlist->AddLine(outer_border.Min,
        ImVec2(outer_border.Min.x, outer_border.Max.y), outer_col, border_size);
      inner_drawlist->AddLine(ImVec2(outer_border.Max.x, outer_border.Min.y),
        outer_border.Max, outer_col, border_size);
    } else if (table->Flags & ImGuiTableFlags_BordersOuterH) {
      inner_drawlist->AddLine(outer_border.Min,
        ImVec2(outer_border.Max.x, outer_border.Min.y), outer_col, border_size);
...
void ImGui::TableFixColumnSortDirection(
  ImGuiTable* table, ImGuiTableColumn* column) {
  if (column->SortOrder == -1 ||
...
void ImGui::TableSetColumnSortDirection(
  ...
  if (column->SortDirection == ImGuiSortDirection_None) column->SortOrder = -1;
...
void ImGui::TableHeadersRow() {
  ...
  ImGuiTable* table = g.CurrentTable;
  IM_ASSERT(
    table != NULL && "Need to call TableHeadersRow() after BeginTable()!");
...
void ImGui::TableLoadSettings(ImGuiTable* table) {
  ...
  for (int data_n = 0; data_n < settings->ColumnsCount;
    ...
    else column->DisplayOrder = (ImGuiTableColumnIdx) column_n;
...
static void TableSettingsHandler_ApplyAll(
  ImGuiContext* ctx, ImGuiSettingsHandler*) {
  ImGuiContext& g = *ctx;
  for (int i = 0; i != g.Tables.GetSize(); i++) {
...
static void* TableSettingsHandler_ReadOpen(
  ...
  if (ImGuiTableSettings* settings = ImGui::TableSettingsFindByID(id)) {
    if (settings->ColumnsCountMax >= columns_count) {
      TableSettingsInit(
        settings, id, columns_count, settings->ColumnsCountMax);  // Recycle
      return settings;
    }
    settings->ID =
      0;  // Invalidate storage, we won't fit because of a count change
...
static void TableSettingsHandler_WriteAll(
  ...
  for (ImGuiTableSettings* settings = g.SettingsTables.begin();
    ...
    buf->reserve(
      buf->size() + 30 + settings->ColumnsCount * 50);  // ballpark reserve
...
float ImGui::GetColumnNormFromOffset(
...
static float GetDraggedColumnOffset(
  ImGuiOldColumns* columns, int column_index) {
...
void ImGui::BeginColumns(
  const char* str_id, int columns_count, ImGuiOldColumnFlags flags) {
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = GetCurrentWindow();
